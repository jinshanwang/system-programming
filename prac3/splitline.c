...
/* this is a modification from the function splitline
  above which return the string array seperated by '|'*/
...
// Purpose:return the length of the commandList
int number_of_arg(char *argv[]){
...
char** execvpArglist(char *commandList){
  char *token;
  char **arg;
  arg = malloc(256);
  token=strtok(commandList," ");
  int number_token=0;
  do {
    arg[number_token]=token;
    // get the next token
    token=strtok(NULL," ");
    number_token++;
  } while(token!=NULL);
  arg[number_token]=NULL;
  free(token);
  return arg;

}
/* isRedir() function:return -1 if there is no REDIRECTION
 or 1 for ">" or 2 for "<"*/
int isRedir(char **command){
  int counter=0;
...
void globbingProcess(char **command) {
...
char** redirec_IN(char **command){
	int i=0;
          while (command[i][0]!='>') {
            i++;
          }
          close(1);
          open(command[i+1],O_CREAT | O_WRONLY,S_IRWXU);
          command[i]=NULL;
    return command;
}
char** redirec_OUT(char **command){
	int i=0;
          while (command[i][0]!='<') {
            i++;
          }
